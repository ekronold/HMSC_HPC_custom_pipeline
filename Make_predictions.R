...
gradientData <- function(hM,
  ...
  if (measure == "Y") {
    ...
    # Extract only focal species from posterior predictions
    # predY is a list of posterior draws:
    # each element = gradient points x species
    ...
    focal_pred <- do.call(cbind, focal_pred)
    
    # Calculate posterior quantiles only for focal species
    qpred <- apply(
      focal_pred,
      1,
      quantile,
